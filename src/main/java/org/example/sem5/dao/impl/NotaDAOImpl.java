...
public class NotaDAOImpl implements NotaDAO {
    ...
    // ✅ Nuevo método para insertar o actualizar automáticamente una nota
    @Override
    public void crearOActualizar(Nota nota) {
        try (Connection conn = ConexionDB.getConnection();
    ...
    public Nota obtenerPorIdDetalleYIdEvaluacion(int idDetalle, int idEvaluacion) {
        Nota nota = null;
        try (Connection conn = ConexionDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(SELECT_BY_DETALLE_EVAL_SQL)) {
    ...
    public List<Nota> obtenerPorIdDetalle(int idDetalle) {
        ...
        try (Connection conn = ConexionDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(SELECT_BY_DETALLE_SQL)) {
            ps.setInt(1, idDetalle);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                notas.add(new Nota(
