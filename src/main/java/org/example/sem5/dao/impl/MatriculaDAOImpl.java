...
public class MatriculaDAOImpl implements MatriculaDAO {
    ...
    private static final String INSERT_SQL = "INSERT INTO Matricula (idAlumno, idPeriodo, fechaMatricula, estado) VALUES (?, ?, ?, ?);";
    private static final String SELECT_BY_ID_SQL = "SELECT * FROM Matricula WHERE idMatricula = ?;";
    private static final String SELECT_BY_ALUMNO_ID_SQL = "SELECT * FROM Matricula WHERE idAlumno = ? ORDER BY idPeriodo DESC;";
    private static final String UPDATE_SQL = "UPDATE Matricula SET idAlumno = ?, idPeriodo = ?, fechaMatricula = ?, estado = ? WHERE idMatricula = ?;";
    private static final String SELECT_ALL_INFO_SQL =
            "SELECT m.idMatricula, m.fechaMatricula, m.estado, " +
    ...
    public Matricula obtenerPorId(int id) {
        ...
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_BY_ID_SQL)) {
            ...
            if (rs.next()) {
                matricula = new Matricula(rs.getInt("idMatricula"), rs.getInt("idAlumno"), rs.getInt("idPeriodo"), rs.getTimestamp("fechaMatricula"), rs.getString("estado"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Matricula> obtenerPorIdAlumno(int idAlumno) {
        ...
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_BY_ALUMNO_ID_SQL)) {
            ps.setInt(1, idAlumno);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                matriculas.add(new Matricula(rs.getInt("idMatricula"), rs.getInt("idAlumno"), rs.getInt("idPeriodo"), rs.getTimestamp("fechaMatricula"), rs.getString("estado")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<MatriculaInfo> obtenerTodaLaInfo() {
        ...
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_ALL_INFO_SQL)) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
