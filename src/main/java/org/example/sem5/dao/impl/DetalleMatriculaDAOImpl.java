...
public class DetalleMatriculaDAOImpl implements DetalleMatriculaDAO {
    ...
    private static final String INSERT_SQL = "INSERT INTO DetalleMatricula (idMatricula, idCurso, estado) VALUES (?, ?, ?);";
    private static final String SELECT_BY_MATRICULA_SQL = "SELECT * FROM DetalleMatricula WHERE idMatricula = ?;";
    private static final String UPDATE_ESTADO_SQL = "UPDATE DetalleMatricula SET estado = ? WHERE idDetalle = ?;";
    private static final String DELETE_SQL = "DELETE FROM DetalleMatricula WHERE idDetalle = ?;";
    ...
    public List<DetalleMatricula> obtenerPorIdMatricula(int idMatricula) {
        ...
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(SELECT_BY_MATRICULA_SQL)) {
    ...
    public List<DetalleMatriculaInfo> obtenerInfoPorIdMatricula(int idMatricula) {
        ...
        String sql = "SELECT dm.idDetalle, c.nombreCurso, c.creditos, dm.estado " +
                "FROM DetalleMatricula dm JOIN Curso c ON dm.idCurso = c.idCurso " +
    ...
    public boolean actualizarEstado(int idDetalle, String nuevoEstado) {
        ...
        try (Connection conn = ConexionDB.getConnection(); PreparedStatement ps = conn.prepareStatement(UPDATE_ESTADO_SQL)) {
